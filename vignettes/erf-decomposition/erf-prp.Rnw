...
opts_chunk$set(fig.path='figure/erf-prp-', cache.path='cache/erf-prp-', 
...
<<prp-setup, include=FALSE, cache=FALSE>>=
...
<<erf,dev='tikz',fig.width=6.27,fig.height=5.5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
gg <- prp("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
    prp.combine.lw.and.sw() %>%
    mutate(pert = sprintf("$F_{\\%s}$", pert)) %>%
    group_by(pert) %>%
    prp.plot(2.5)
...
\begin{figure*}[t]
  ...
  <<erf-susc,dev='tikz',fig.width=6.27,fig.height=5.5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=

  df.ln.Nd <- delta.ln.nd("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly")
  
  df.suscept <- prp("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
      prp.combine.lw.and.sw() %>%
      full_join(df.ln.Nd, by = c("lon", "lat")) %>% 
      mutate(pert = sprintf("$F_{\\%s}/\\Delta \\ln \\overline{\\mathcal{N}}_d$", pert)) %>%
      group_by(pert, lon, lat) %>%
      mutate(erf = ifelse(delta.ln.nd > 0.05, erf / delta.ln.nd, NA)) %>%
      filter(!is.na(erf)) %>%
      ungroup()

  gg.suscept <- df.suscept %>% 
      group_by(pert) %>% 
      prp.plot(8, palette = "YlGnBu", symmetric = FALSE)

  gg.suscept + theme(axis.title.x = element_blank(), axis.title.y = element_blank())
...
\begin{figure}[t]
  ...
  <<delta-ln-nd-filled-contour,dev='tikz',fig.width=3.15,fig.height=2.4,out.width='0.5\\linewidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
  
  range <- 2
  delta.ln.nd("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
      mutate(lon = ifelse(lon <= 180, lon, lon - 360)) %>%
      mutate(dln = delta.ln.nd) %>%
      ## discretize(delta.ln.nd, seq(-0.125,1.875,0.25)) %>%
      ## mutate(delta.ln.nd = factor(delta.ln.nd)) %>% 
      ggplot(aes(lon, lat)) + ## pmax(pmin(delta.ln.nd, range), -range))) +
      geom_contour_fill(aes(z = dln), binwidth = 0.25) +
      ## geom_raster() +
      ## geom_contour(aes(z = as.integer(delta.ln.nd)), lwd = 0.5) +
      ...
      scale_fill_distiller("$\\Delta\\ln\\mathcal{N}_d$ ", palette = "YlOrBr", direction = 1
                         , breaks = MakeBreaks(0.25)
                         , labels = tikz_sanitize_sparse
                           ##, limits = c(-1, 1) * range
                         , guide = guide_colorstrip(title.vjust = 0.75, title.hjust = 0, barwidth = 8)
                           ) +
      ## geom_contour(aes(z = delta.ln.nd), binwidth = 0.1) +
      ...
      ##                            label.hjust = 0.5,
      ##                            title.position = "left",
      ##                            title.vjust = 0.75, barwidth = 8)) +
      ## guides(fill = guide_colorstrip(direction = "horizontal", title.vjust = 0.75, barwidth = 8)) +
...
<<erf-ratios,dev='tikz',fig.width=3.15,fig.height=3,out.width='8cm',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
prp("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
    prp.combine.lw.and.sw() %>%
    discretize(lat, seq(-90,90,5)) %>%
    group_by(lat, pert) %>%
    summarize(erf = mean(erf)) %>%
    ungroup() %>%
    spread(pert, erf) %>%
    mutate(cf = cf / cdnc, 
           lwp = lwp / cdnc) %>%
    select(-cdnc) %>%
    gather(pert, erf, cf, lwp) %>%
    mutate(pert = sprintf("$\\xi = \\%s$", pert)) %>%
    ggplot(aes(lat, erf, lty = pert)) + 
    geom_line() +
    scale_x_geo_zonmean() +
    scale_linetype("") +
    labs(y = "$F_\\xi/F_{\\nd}$") +
    theme_bw()  + theme(axis.title.x = element_blank()) + theme(legend.position = "bottom", legend.box = "horizontal") 
...
<<erf-regress,dev='tikz',fig.width=6.27,fig.height=3.5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
prp("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
    prp.combine.lw.and.sw() %>%
    filter(erf > -2, erf < 0.4) %>%
    spread(pert, erf) %>%
    gather(pert, erf, lwp, cf) %>%
    mutate(pert = sprintf("$\\xi = \\%s$", pert)) %>%
    ggplot(aes(cdnc, erf)) +
    stat_bin2d(geom = "raster", binwidth = 0.05) + 
    stat_smooth(method = "lm", col = "red", fill = "red", formula = y ~ x) +
    ggplot2::stat_smooth(method = "auto", col = "blue", fill = "blue", formula = y ~ x) +
    ggplot2::geom_abline(intercept = 0, slope = 1, lty = "dashed", col = "lightgrey") +
    ## ggplot2::coord_fixed(1e-3, xlim = c(0,3), ylim = c(0,3000), expand = FALSE) +
    labs(x = "$F_{\\nd}$ (\\unit{W~m^{-2}})", y = "$F_\\xi$ (\\unit{W~m^{-2}})") +
    facet_grid(. ~ pert) +
    coord_equal(expand = FALSE) + 
    scale_fill_distiller("Count",
                         palette = "GnBu",
...
<<erf-fw-pd-pi,dev='tikz',fig.width=6.27,fig.height=5.5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
gg <- bind_rows(prp.fw("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
                mutate(dir = "(a) Forward: $\\overline{\\delta_{\\text{PD}\\rightarrow\\text{PI}}(Q+R)_\\xi}{}^{(3~\\text{h})}$"),
                prp.bk("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
                mutate(dir = "(b) Backward: $\\overline{\\delta_{\\text{PI}\\rightarrow\\text{PD}}(Q+R)_\\xi}{}^{(3~\\text{h})}$")) %>%
    prp.combine.lw.and.sw() %>%
    mutate(pert = sprintf("$\\xi = \\%s$", pert)) %>%
    group_by(pert, dir) %>%
    prp.plot(8)
...
<<erf-fw,dev='tikz',fig.width=6.27,fig.height=4.5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
gg <- bind_rows(prp.fw("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta") %>%
                mutate(dir = "(a) Forward:\\,$\\overline{\\delta_{\\beta\\rightarrow\\beta'}(Q+R)_\\xi}{}^{(3\\,\\text{h})}$"),
                prp.bk("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta") %>%
                mutate(dir = "(b) Backward:\\,$\\overline{\\delta_{\\beta'\\rightarrow\\beta}(Q+R)_\\xi}{}^{(3\\,\\text{h})}$"),
                prp("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta") %>%
                mutate(dir = "(c) Forward--Backward:\\,$\\tilde{F}_{\\xi}$")) %>%
    prp.combine.lw.and.sw() %>%
    mutate(pert = sprintf("$\\xi = \\%s$", pert)) %>%
    group_by(pert, dir) %>%
    prp.plot(8)
...
<<erftot-setup,dev='tikz',fig.width=6.27,fig.height=3.5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
## the 3-hourly experiment was run with just "hourly" in the
## experiment name, so the rad-summary files say "hourly" too... but
## they are the same runs as were used for
## ~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly below
df.rad <- readRDS("~/wcrain/echam-ham/rad-summary-rain_4_1_-1_no-cosp_nudged_hourly.rds") %>%
    group_by(lon, lat) %>%
    summarize(srad0 = mean(srad0),
              trad0 = mean(trad0),
              sraf0 = mean(sraf0),
              traf0 = mean(traf0),
              aclcov = mean(aclcov)) %>%
...
df.rad.pi <- readRDS("~/wcrain/echam-ham/rad-summary-rain_4_1_-1_pi_no-cosp_nudged_hourly.rds") %>%
    group_by(lon, lat) %>%
    summarize(srad0 = mean(srad0),
              trad0 = mean(trad0),
              sraf0 = mean(sraf0),
...
df.rad.diff <- bind_rows(df.rad %>%
                         select(lon, lat, srad0 : aclcov) %>%
                         gather(type, flux, srad0 : traf0) %>%
                         group_by(type) %>%
			 filter(abs(lat) <= 60) %>%
			 mutate(cos.lat = cos(lat * pi / 180)) %>%
			 summarize(flux = sum(flux * cos.lat, na.rm = TRUE) / sum(cos.lat)) %>%
                         mutate(pd_pi = "PD"),
                         df.rad.pi %>%
                         select(lon, lat, srad0 : aclcov) %>%
                         gather(type, flux, srad0 : traf0) %>%
...
<<prp-3h-global,dev='tikz',fig.width=12,fig.height=5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
    ## prp.combine.lw.and.sw() %>%
    rename(Spectrum = spectrum) %>%
    ## "rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_all_cf_0.99") %>%
##    group_by(beta) %>%
    spread(pert, erf) %>%
    mutate(sum = cdnc + lwp + cf) %>%
    gather(pert, erf, cdnc, lwp, cf, sum) %>%
    mutate(pert = sprintf("$F_{\\%s}$", pert)) %>%
    spread(pert, erf) %>%
    rename("\\multicolumn{1}{c|}{$F_{\\cdnc} + F_{\\cf} + F_{\\lwp}$}" = "$F_{\\sum}$") %>%
    ## ## bind total ERF from trad0/srad0 (but I accidentially picked the wrong
    ## ## column in an old table, using the SW ERF in two different ccraut
    ## ## configurations)
    ## ## bind_cols(" " = c(0.86,-1.10)) %>%
    ## bind total ERF from trad0/srad0 rad-summary.rds:
    bind_cols(" " = c(df.rad.diff$delta[df.rad.diff$type == "trad0"], 
                      df.rad.diff$delta[df.rad.diff$type == "srad0"])) %>%
    xtable::xtable() %>%
...
\begin{table*}[t]
  ...
  \hline\hline
    & \multicolumn{3}{c}{ERF components (W m$^{-2}$)}\\
<<prp-global-phase,dev='tikz',fig.width=12,fig.height=5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
df.ice.frac <- readRDS("ice-frac.rds")

bind_rows(prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_daily_2000-2005_iliqcld_0") %>%
          mutate(order = 0, "Phase treatment" = "All phases"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_daily_2000-2005") %>%
          mutate(order = 2, "Phase treatment" = "Liquid-only cloudy model levels"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_daily_2000-2005_iliqcld_3") %>%
          mutate(order = 3, "Phase treatment" = "Liquid-only cloudy model columns"),
          ## apply postprocessing to ice-free columns to correct for time fraction
          prp("~/echam-prp/rain_4_1_-1_no-cosp_nudged_daily_2000-2005_iliqcld_3") %>%
          full_join(df.ice.frac, by = c("lon", "lat")) %>%
          filter(abs(lat) <= 60) %>%
          mutate(cos.lat = cos(lat * pi / 180)) %>%
          group_by(pert, spectrum) %>%
          summarize(erf = sum(erf / (1 - ice.frac) * cos.lat, na.rm = TRUE) / sum(cos.lat * is.finite(erf))) %>%
          mutate(order = 4, "Phase treatment" = "Liquid-only cloudy model columns (corrected for occurrence fraction)")) %>%
    filter(spectrum == "SW") %>%
    select(-spectrum) %>%
    ## "rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_all_cf_0.99") %>%
##    group_by(beta) %>%
    mutate(pert = sprintf("$\\delta Q_{\\%s}$", pert)) %>%
    spread(pert, erf) %>%
    select(-order) %>%
    as.data.frame() %>%
    xtable::xtable() %>%
...
<<ghan-decomposition,dev='tikz',fig.width=12,fig.height=5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='hide'>>=

df.ghan <- bind_rows(read.csv("pnas.1514036113.pd.csv") %>% mutate(pd_pi = "PD"),
                     read.csv("pnas.1514036113.pi.csv") %>% mutate(pd_pi = "PI"))

df.ghan %<>%
    gather(var, val, lwp:Rc) %>%
    spread(pd_pi, val) %>%
    group_by(Model) %>% 
    mutate(delta = PD - PI,
           delta.ln = log(PD/PI)) %>%
    ungroup() %>% 
    gather(statistic, val, PD, PI, delta, delta.ln) %>%
    mutate(var = sprintf("%s_%s", var, statistic)) %>%
    select(-statistic) %>% 
    spread(var, val)

df.ghan %>%
    mutate(fc = SWCF_PI * lcc_delta.ln,
           lwp = SWCF_PI * Rc_delta.ln / cod_delta.ln * lwp_delta.ln,
           cdnc = -SWCF_PI * Rc_delta.ln / cod_delta.ln * cdr_delta.ln) %>%
    select(Model, fc, lwp, cdnc)
...
\begin{table}[t]
  ...
  \hline\hline
    & \multicolumn{3}{c}{ERF components (W m$^{-2}$)}\\
<<prp-global-temporal,dev='tikz',fig.width=12,fig.height=5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
bind_rows(prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged") %>%
          mutate(order = 1, "Averaging period" = "1~\\unit{month}"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_daymean") %>%
          mutate(order = 2, "Averaging period" = "1~\\unit{d}"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly") %>%
          mutate(order = 3, "Averaging period" = "3~\\unit{h}"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_inst") %>%
          mutate(order = 4, "Averaging period" = "instantaneous")) %>%
    prp.combine.lw.and.sw() %>%
    ## "rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_all_cf_0.99") %>%
##    group_by(beta) %>%
    mutate(pert = sprintf("$F_{\\%s}$", pert)) %>%
...
\begin{table*}[t]
 ...
 \lwp, and \cf; estimate of the same \erfaci\ components by
  ...
  \hline\hline
    & \multicolumn{3}{c}{ERF components (W m$^{-2}$)}\\
<<prp-global-pert,dev='tikz',fig.width=12,fig.height=5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
bind_rows(prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_clone_lwp") %>%
          mutate(order = 3, Forcing = "$\\lwp\\times 1.1$"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_clone_cdnc") %>%
          mutate(order = 1, Forcing = "$\\nd\\times 1.1$"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_clone_cf_0.99") %>%
          mutate(order = 2, Forcing = "$\\cf\\times 0.99$"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_lwp") %>%
          mutate(order = 6, Forcing = "$\\lwp\\times 1.1$ with $\\beta' = \\beta + 10^{-5}$"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_cdnc") %>%
          mutate(order = 4, Forcing = "$\\nd\\times 1.1$ with $\\beta' = \\beta + 10^{-5}$"),
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_cf_0.99") %>%
          mutate(order = 5, Forcing = "$\\cf\\times 0.99$ with $\\beta' = \\beta + 10^{-5}$"),          
          prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_all_cf_0.99") %>%
          mutate(order = 7, Forcing = "$\\nd\\times 1.1, \\lwp\\times 1.1, \\cf\\times 0.99$ with $\\beta' = \\beta + 10^{-5}$")) %>%
    mutate(Forcing = factor(Forcing, ordered = TRUE)) %>%
    rename(Perturbation = Forcing) %>% 
    prp.combine.lw.and.sw() %>%
    ## "rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_all_cf_0.99") %>%
##    group_by(beta) %>%
    mutate(pert = sprintf("$F_{\\%s}$", pert)) %>%
    ...
    print(., sanitize.colnames.function = identity,
          ...
          NA.string = "NA",
          format.args = list(flag = "#"),
          hline.after = c(0, 3, 6, rep(nrow(.), 2)))
@ 
  \end{tabular}
\end{table*}
...
\begin{table}[t]
  ...
  \hline\hline
    \multicolumn{3}{c}{ERF components (W m$^{-2}$)}\\
<<prp-3h-global-incloud,dev='tikz',fig.width=12,fig.height=5,out.width='\\textwidth',message=FALSE,cache=TRUE,echo=FALSE,results='asis'>>=
prp.global("~/echam-prp/rain_4_1_-1_no-cosp_nudged_3hourly_in-cloud_2000") %>%
    prp.combine.lw.and.sw() %>%
    ## "rain_4_1_-1_no-cosp_nudged_3hourly_pert_cautbeta_all_cf_0.99") %>%
##    group_by(beta) %>%
    mutate(pert = sprintf("$F_{\\%s}$", pert)) %>%
